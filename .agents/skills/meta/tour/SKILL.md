...
This repo is your campaign brain. The world lives in `context/` as Markdown. Past sessions live in `sessions/`. Skills under `.agents/skills/` (also discoverable via `.claude/skills/`) do the repeatable DM work for you — both deskwork between sessions and quick rulings at the table. Nothing is auto-magic: you stay the DM, the OS just remembers, drafts, and proposes.
...
**Prep skills** (`.agents/skills/prep/`) are for **deskwork** — between sessions, with time to think.
...
- Use them for: building NPCs, designing encounters, writing locations, growing the world bible, tracking quest threads, prepping or recapping a session, advancing PC arcs, rebuilding a stale `_INDEX.md`.

**Live skills** (`.agents/skills/live/`) are for **at the table** — mid-session, time pressure.
...
- Use them for: improvising an NPC, naming a region, rolling a random encounter, scaling an encounter on the fly, looking up a 5e rule, looking up a spell, pulling a monster stat block, deciding who knows what.
...
context/                     campaign canon — the current state of the world
  ...
  rules/srd/                 vendored 5e SRD (CC-BY-4.0) — read by 5e-rules, spell-lookup, statblock
  maps/                      committed image files

sessions/                    chronological log, one file per session
  NNN-headline.md            raw notes + recap + state deltas

.agents/skills/              the agent skills (also reachable via .claude/skills/ symlink)
  prep/                      deskwork skills (rich, propose-then-write)
  live/                      at-the-table skills (terse, no writes)
  meta/                      skills about the OS itself (this tour lives here)

docs/superpowers/            design and plan docs (meta — not gameplay)
TODO.md                      upcoming features and unsolved problems
...
## The skills, with their fire phrases

### Prep skills (8) — `.agents/skills/prep/`
...
| `reindex` | "rebuild the npcs index", "the locations _INDEX is out of date" |

### Live skills (9) — `.agents/skills/live/`
...
| `spell-lookup` | "what does Hypnotic Pattern do?", "Hold Person — give me the text" |
| `statblock` | "give me the stats for a cultist", "AC and HP of a goblin boss" |
...
2. **During the session — play**
   - Keep the prep doc open. When the players go off-script, call live skills:
     ...
     - "stats for a goblin boss"
     - "what does Counterspell actually do"
...
5. **Open one prep skill** (e.g. `.agents/skills/prep/sessions/SKILL.md`) and **one live skill** (e.g. `.agents/skills/live/improvise-npc/SKILL.md`). Compare their length and tone — that contrast is the OS's spine.
6. **Try a live skill cold:** ask "improvise an NPC: drunk dockhand at the Rusted Anchor." You should get ~6 lines, no files touched.
7. **Try a rules lookup:** ask "how does grappling work?" — should pull from `context/rules/srd/Gameplay/Combat.md` and quote the actual SRD text.
8. **Try a prep skill:** ask "draft an NPC: a retired bounty hunter who runs the Hollowgate stables." It should propose a file path and full content, then wait for your approval.
9. **Approve the proposal** and watch it write `context/npcs/<slug>.md` plus a one-row update to `context/npcs/_INDEX.md`. That's the loop.
...
- **Search feels broken.** Check the relevant `_INDEX.md` — a missing or stale row is almost always the cause. Run `prep/reindex` to rebuild.
- **A rules answer is labeled `(unverified — model knowledge)`.** That means `5e-rules` couldn't find the topic in the vendored SRD and fell back to model recall. Double-check the answer.
